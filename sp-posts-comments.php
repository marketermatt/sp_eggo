...
if (!function_exists('sp_continue_reading_link')) {
	function sp_continue_reading_link() {
		return ' <a href="'. get_permalink() . '" class="more">' . __( 'Read More &gt;', 'sp' ) . '</a>';
	}
}
...
function sp_related_post() {
	...
	if (sp_isset_option( 'blog_sidebar_orientation', 'value' ) == 'no-sidebars' ) {
		$show_posts = 5;
	} else {
		$show_posts = 4;	
	}
	...
	if ($tags) {
		...
		if( $my_query->have_posts() ) {
			...
			?>
				<li><div class="wrap"><img width="<?php echo $image_width; ?>" height="<?php echo $image_height; ?>" class="wp-post-image" alt="<?php the_title_attribute(); ?>" src="<?php echo sp_timthumb_format( 'blog_related', $post_image_url, $image_width, $image_height ); ?>" /><a href="<?php the_permalink() ?>" data-rel="bookmark" title="Permanent Link to <?php the_title_attribute(); ?>" class="more"><span class="icon">&nbsp;</span><span><?php _e('Read More','sp'); ?></span></a></div><h3><?php the_title(); ?></h3></li>
			<?php
				} else { ?>
				<li><div class="wrap"><img width="<?php echo $image_width; ?>" height="<?php echo $image_height; ?>" class="wp-post-image" alt="<?php the_title_attribute(); ?>" src="<?php echo sp_timthumb_format( 'blog_related', get_template_directory_uri().'/images/no-product-image.jpg', $image_width, $image_height ); ?>" /><a href="<?php the_permalink() ?>" data-rel="bookmark" title="Permanent Link to <?php the_title_attribute(); ?>" class="more"><span class="icon">&nbsp;</span><span><?php _e('Read More','sp'); ?></span></a></div><h3><?php the_title(); ?></h3></li>
                <?php
				}
			endwhile;
			echo '</ul></div>';
