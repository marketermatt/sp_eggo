...
if(wpsc_cart_item_count() < 1) :
   _e('Oops, there is nothing in your cart.', 'sp') . "<a href=".get_option("product_list_url").">" . __('Please visit our shop', 'sp') . "</a>";
   return;
endif;
...
if (!empty($_SESSION['wpsc_checkout_error_messages']) || !empty($_SESSION['wpsc_checkout_misc_error_messages']) || !empty($_SESSION['wpsc_checkout_user_error_messages']) || isset($_POST['wpsc_submit_zipcode']) || isset($_POST['wpsc_gateway_error_messages']) || isset($_POST['wpsc_update_location']) || (isset($_GET['login']) && $_GET['login'] == 1)) : ?>
	...
	jQuery(document).ready(function() {
		jQuery(".slide1").hide();
		jQuery(".slide2").show();
		jQuery("#status-bg .cart").addClass("active");
...
<div class="slide1">
	<div class="yourtotal"><p class="title"><?php _e('Sub-Total', 'sp'); ?></p><p class="total"><?php echo wpsc_cart_total_widget(false,false,false); ?></p></div><!--close yourtotal-->
